...
func GetPersistentFlags(cmd *cobra.Command, colorSupport bool, colorCount int, username string) {
	...
	cmd.PersistentFlags().BoolVarP(&flagShowAll, "all", "", false, "equivalent to -a --age -c -g -m -p -t")
	...
	cmd.PersistentFlags().StringVarP(&flagContains, "contains", "s", "", "show only branches containing process with <pattern> in commandline")
