...
func GetPersistentFlags(cmd *cobra.Command, colorSupport bool, colorCount int, username string) {
	...
	if colorSupport {
		...
		if colorCount >= 8 && colorCount < 256 {
			cmd.PersistentFlags().BoolVarP(&flagColorize, "colorize", "", false, fmt.Sprintf("add some %s to the output; cannot be used with --color or --rainbow", util.Color8()))
		} else if colorCount >= 256 {
			cmd.PersistentFlags().BoolVarP(&flagColorize, "colorize", "", false, gorainbow.Rainbow("add some beautiful color to the pstree output; cannot be used with --color or --rainbow"))
			cmd.PersistentFlags().BoolVarP(&flagRainbow, "rainbow", "", false, "please don't; cannot be used with --color or --colorize")
		}
	}
