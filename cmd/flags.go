...
func GetPersistentFlags(cmd *cobra.Command, colorSupport bool, colorCount int) {
	...
	cmd.PersistentFlags().BoolVarP(&flagThreads, "threads", "t", false, "show the thread count with each process, e.g., (t: xx)")
	...
	cmd.PersistentFlags().BoolVarP(&flagNoPids, "no-pids", "", false, "do not show PIDs")
